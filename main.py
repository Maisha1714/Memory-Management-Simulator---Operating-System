...
def visualize_memory(memory_blocks, title):
    fig, ax = plt.subplots()
    bars = ax.bar(range(len(memory_blocks)), memory_blocks, color='blue')
    plt.xlabel('Memory Blocks')
    plt.ylabel('Block Size')
    plt.title(title)
    cursor = mplcursors.cursor(bars, hover=True)
    cursor.connect("add", lambda sel: sel.annotation.set_text(f'Block {sel.index}: {memory_blocks[sel.index]}'))
    plt.show()
